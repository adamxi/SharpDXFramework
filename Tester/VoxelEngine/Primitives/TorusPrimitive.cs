...
namespace VoxelEngine.Primitives
{
	...
	public class TorusPrimitive : GeometricPrimitive
	{
		...
		/// </summary>
		public TorusPrimitive() : this( 1, 0.333f, 32 ) { }
		...
		public TorusPrimitive( float diameter, float thickness, int tessellation )
		{
			...
			{
				float outerAngle = i * MathUtil.TwoPi / tessellation;

				// Create a transform matrix that will align geometry to
				// slice perpendicularly though the current ring position.
				Matrix transform = Matrix.Translation( diameter / 2, 0, 0 ) * Matrix.RotationY( outerAngle );

				// Now we loop along the other axis, around the side of the tube.
				for( int j = 0; j < tessellation; j++ )
				{
					float innerAngle = j * MathUtil.TwoPi / tessellation;
