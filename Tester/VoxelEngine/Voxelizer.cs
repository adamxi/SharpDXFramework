...
namespace VoxelEngine
{
	...
	{
		// The Length of the grid on each side
		private const int CHUNK_SIZE = 100;

		// The Array of all the GridCells
		private static GridCell[ , , ] grids;
		...
		public static void Init()
		{
			...
			density = noise.Generate3D( len, len, len, 0, 0, 0 );

			// Create the array of gridcells at 1 less than the length
			grids = new GridCell[ CHUNK_SIZE, CHUNK_SIZE, CHUNK_SIZE ];

			for( int x = CHUNK_SIZE; --x >= 0; )
			{
				for( int y = CHUNK_SIZE; --y >= 0; )
				{
					for( int z = CHUNK_SIZE; --z >= 0; )
					{
						grids[ x, y, z ] = GenerateGridCell( x, y, z ); // For every gridcell in the array generate based on its x, y, and z co-ordinates
					}
				}
			}

			p.Stop();
			Console.WriteLine( "Voxelizer initialization: " + p.ShortOutput() );
		}

		public static VoxelChunk CreateChunk( float isolevel )
		{
			Profiler p = Profiler.StartNew();
			...
			//		{
			//			Triangle[] triangles = GenerateTriangles( isolevel, grids[ x, y, z ] );
			//			Build( chunk, ref triangles );
			//		}
			//	}
			//}
			for( int x = CHUNK_SIZE; --x >= 0; )
			...
			p.Stop();
			Console.WriteLine( "Voxel chunk generation: " + p.ShortOutput() );
			return chunk;
		}
		...
		/// <param name="grid">The GridCell to generate the triangles for</param>
		/// <returns></returns>
		private static Triangle[] GenerateTriangles( float isolevel, GridCell grid )
		{
			...
			if( ( edgeTable[ cubeindex ] & 1 ) != 0 )
				VertexInterp( ref isolevel, ref p[ 0 ], ref p[ 1 ], ref vals[ 0 ], ref vals[ 1 ], out vertlist[ 0 ] );
			if( ( edgeTable[ cubeindex ] & 2 ) != 0 )
				VertexInterp( ref isolevel, ref p[ 1 ], ref p[ 2 ], ref vals[ 1 ], ref vals[ 2 ], out vertlist[ 1 ] );
			if( ( edgeTable[ cubeindex ] & 4 ) != 0 )
				VertexInterp( ref  isolevel, ref p[ 2 ], ref p[ 3 ], ref vals[ 2 ], ref vals[ 3 ], out vertlist[ 2 ] );
			if( ( edgeTable[ cubeindex ] & 8 ) != 0 )
				VertexInterp( ref  isolevel, ref p[ 3 ], ref p[ 0 ], ref  vals[ 3 ], ref vals[ 0 ], out vertlist[ 3 ] );
			if( ( edgeTable[ cubeindex ] & 16 ) != 0 )
				VertexInterp( ref isolevel, ref p[ 4 ], ref p[ 5 ], ref  vals[ 4 ], ref vals[ 5 ], out vertlist[ 4 ] );
			if( ( edgeTable[ cubeindex ] & 32 ) != 0 )
				VertexInterp( ref isolevel, ref p[ 5 ], ref p[ 6 ], ref  vals[ 5 ], ref vals[ 6 ], out vertlist[ 5 ] );
			if( ( edgeTable[ cubeindex ] & 64 ) != 0 )
				VertexInterp( ref isolevel, ref p[ 6 ], ref  p[ 7 ], ref vals[ 6 ], ref vals[ 7 ], out vertlist[ 6 ] );
			if( ( edgeTable[ cubeindex ] & 128 ) != 0 )
				VertexInterp( ref isolevel, ref p[ 7 ], ref p[ 4 ], ref  vals[ 7 ], ref vals[ 4 ], out vertlist[ 7 ] );
			if( ( edgeTable[ cubeindex ] & 256 ) != 0 )
				VertexInterp( ref isolevel, ref p[ 0 ], ref  p[ 4 ], ref  vals[ 0 ], ref vals[ 4 ], out vertlist[ 8 ] );
			if( ( edgeTable[ cubeindex ] & 512 ) != 0 )
				VertexInterp( ref isolevel, ref p[ 1 ], ref p[ 5 ], ref  vals[ 1 ], ref vals[ 5 ], out vertlist[ 9 ] );
			if( ( edgeTable[ cubeindex ] & 1024 ) != 0 )
				VertexInterp( ref  isolevel, ref p[ 2 ], ref  p[ 6 ], ref vals[ 2 ], ref vals[ 6 ], out vertlist[ 10 ] );
			if( ( edgeTable[ cubeindex ] & 2048 ) != 0 )
				VertexInterp( ref  isolevel, ref p[ 3 ], ref p[ 7 ], ref vals[ 3 ], ref vals[ 7 ], out vertlist[ 11 ] );
			...
			for( int i = t.Length; ( i -= 3 ) >= 0; )
			{
				triangles[ index-- ] = new Triangle( vertlist[ t[ i ] ], vertlist[ t[ i + 1 ] ], vertlist[ t[ i + 2 ] ] );
			}
		...
		private static GridCell GenerateGridCell( int x, int y, int z )
		{
			// Find the densities on each corner of the grid cell
			float[] denses = new float[]{
				density[ x, y, z + 1 ],
				density[ x + 1, y, z + 1 ],
				density[ x + 1, y, z ],
				density[ x, y, z ],
			...
			// Find the positions of each corner of the grid cell
			float xSize = x * gridSize;
			float ySize = y * gridSize;
			float zSize = z * gridSize;

			Vector3[] positions = new Vector3[]{
				new Vector3( xSize, ySize, zSize + gridSize ),
				new Vector3( xSize + gridSize, ySize, zSize + gridSize ),
				new Vector3( xSize + gridSize, ySize, zSize ),
				new Vector3( xSize, ySize, zSize ),
				new Vector3( xSize, ySize + gridSize, zSize + gridSize ),
				new Vector3( xSize + gridSize, ySize + gridSize, zSize + gridSize ),
				new Vector3( xSize + gridSize, ySize + gridSize, zSize ),
				new Vector3( xSize, ySize + gridSize, zSize)};

			// Return the finished grid cell
			return new GridCell( positions, denses );
		}
		...
		/// <returns></returns>		
		private static void VertexInterp( ref float isolevel, ref Vector3 p1, ref Vector3 p2, ref float valp1, ref float valp2, out Vector3 p )
		{
